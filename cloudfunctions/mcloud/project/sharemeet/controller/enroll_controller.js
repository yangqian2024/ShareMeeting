...
class EnrollController extends BaseProjectController {
	...
	async getMyEnrollJoinList() {
		...
		for (let k = 0; k < list.length; k++) {
			...
			list[k].ENROLL_JOIN_DAY_DESC = year[0] + '.' + year[1] + '.' + year[2];

			list[k].week = timeUtil.week(list[k].ENROLL_JOIN_DAY);

			list[k].expire = timeUtil.time2Timestamp(list[k].ENROLL_JOIN_DAY + ' ' + list[k].ENROLL_JOIN_END_POINT) < timeUtil.time();
	...
	async getMyEnrollJoinDetail() {
		...
		if (enrollJoin) {
			...
			enrollJoin.ENROLL_JOIN_DAY_DESC = year[0] + '.' + year[1] + '.' + year[2];
